...
The current version is a basic implementation to test out the feasibility of the conceot, and explore how to make a consistent unified API across platforms.
...
if (SSBX.isAvailable()) {
  SSBX.log('hello');
  SSBX.notify('This is a message', 'A message from SSBX');
  SSBX.notifyOnce('5', 'message', 'title');
  SSBX.setDockBadge(5);
...
SSBX requires Prototype (prototypejs.org).

h3. Supported SSBs

Windows: <a href="http://bubbleshq.com/api">Bubbles</a>
Mac OS: <a href="http://fluidapp.com/">Fluid</a>

h3. Notification API

Show a notification (requires Growl in Mac OS).  Title is optional.

<code>SSBX.notify(message, title)</code>

Show a notification and log it so it doesn't get displayed again.  Logs to a cookie:

<code>SSBX.notifyOnce(unique_id, message, title)</code>
